...
public:
    ...
    */
            ...
            int& beam_number)
    {
       ...
       //get position
       //start
       tf::Vector3 pos_start = frame_start_pose.getOrigin();
       pos_start.setZ(0);
       //end
       tf::Vector3 pos_end = frame_end_pose.getOrigin();
       pos_end.setZ(0);
       //base_frame
       ...
       tf::Quaternion base_q = frame_base_pose.getRotation();

       double start_angle = tf::getYaw(start_q);
       double end_angle = tf::getYaw(end_q);
       double theta = tf::getYaw(base_q);

       double step = 1.0 / (beam_number - 1);
       ...
       {
           tf::Quaternion angle_slerp = start_q.slerp(end_q, step * i);
           mid_angle = tf::getYaw(angle_slerp);
           mid_pos = pos_start.lerp(pos_end, step * i);

           if(ranges[startIndex + i] < 0.05 || std::isnan(ranges[startIndex + i]) || std::isinf(ranges[startIndex + i]))
               continue;
           ...
           lidar_x = lidar_dist * cos(lidar_angle);
           lidar_y = lidar_dist * sin(lidar_angle);

           //at odom
           double odom_x, odom_y;
           odom_x = lidar_x * cos(mid_angle) - lidar_y * sin(mid_angle) + mid_pos.x();//x * cos(visualYaw) - y * sin(visualYaw) + visualPose.getOrigin().getX();
           odom_y = lidar_x * sin(mid_angle) + lidar_y * cos(mid_angle) + mid_pos.y();// x * sin(visualYaw) + y * cos(visualYaw) + visualPose.getOrigin().getY();
           ...
           //odom_x = x * cos(theta) - y * sin(theta) + x_0;
           //odom_y = x * sin(theta) + y * cos(theta) + y_0;
           //联立方程组求解得:
           //x = odom_x * cos(theta) + odom_y * sin(theta) - x_0 * cos(theta) - y_0 * sin(theta);
           //y = -odom_x * sin(theta) + odom_y * cos(theta) + x_0 * sin(theta) - y_0 * cos(theta);

           double base_x = odom_x * cos(theta) + odom_y * sin(theta) - x_0 * cos(theta) - y_0 * sin(theta);
           double base_y = -odom_x * sin(theta) + odom_y * cos(theta) + x_0 * sin(theta) - y_0 * cos(theta);

           lidar_dist = sqrt(base_x * base_x + base_y * base_y);
           lidar_angle = atan2(base_y, base_x);
